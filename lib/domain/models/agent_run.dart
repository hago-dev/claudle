...
/// 서브에이전트 1회 실행 = `agent-<agentId>.jsonl` 파일 하나.
///
/// 과금 집계([UsageEvent]) 와는 별개 축이다. 이쪽은 "누가 무슨 일을 했나"(시각화)용
/// 읽기 전용 뷰라 DB 를 거치지 않고 파일에서 직접 만든다.
class AgentRun {
  final String agentId; // 파일명 `agent-<agentId>.jsonl` 에서 추출
  final String agentType; // meta.json 의 agentType. 없으면 'unknown'
  final String project; // projects/ 하위 인코딩된 디렉토리명
  final String sessionId; // 이 에이전트를 띄운 세션
  ...
  /// 이 마리가 받은 일 — meta.json 의 라벨, 없으면 첫 프롬프트에서 뽑은 것.
