...
bash tool/release_adhoc.sh                 # 실배포 경로: 게이트 3종 → release/v<버전> 브랜치 → 재빌드 + ad-hoc 재서명 → zip → 원래 브랜치 복귀
```
...
| ② `release/v<버전>` 브랜치 존재 = 그 버전 이미 냄 | `pubspec.yaml` version과 `claudle.iss` AppVersion을 **함께** 올린 뒤 재실행. 어느 버전으로 올릴지가 자명하지 않으면 사용자에게 묻는다 |
| ③ `dist/…v<버전>.zip` 존재 | 덮어쓰기 금지. 의도적 재빌드인지 사용자에게 확인받고, 승인 시에만 지운다 |

**게이트를 끄거나 우회하는 코드를 추가하지 마라** — 게이트가 있는 이유는 내용이 다른 같은 번호 배포본이 앞 것을 덮어쓴 사고(2026-07-17, v1.2.0)다. `dist/`는 gitignore라 복구가 안 되고, `release/v<버전>` 브랜치가 유일한 "이 zip이 어느 소스에서 나왔나" 기록이다(로컬 전용 — push는 명시 요청 시에만).
...
## 배포 전 체크리스트
- [ ] **버전이 이 배포에 쓸 수 있는 번호인가** — `git branch --list 'release/v*'`로 이미 낸 버전 확인. pubspec ↔ iss가 서로 일치해도 그 번호가 이미 나갔으면 못 쓴다(둘은 다른 검사다)
- [ ] macOS: `bash tool/release_adhoc.sh` 산출물 확인 — 게이트 3종 통과 + `codesign --verify --deep --strict` 통과 + `dist/Claudle-macOS-v<버전>.zip` 생성 + `release/v<버전>` 브랜치 생성 + 원래 브랜치로 복귀됨
